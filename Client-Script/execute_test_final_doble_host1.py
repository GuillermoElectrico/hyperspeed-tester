...
def pingHome():
    ScreenOutput('Ping Test Core1', 'Executing...')
    time.sleep(1)
    ...
    if response == 0:
      ...
      status = True
      ScreenOutput('Ping Test Core1', 'Succesful')
      time.sleep(1)
    else:
      print hostname, 'is down!'
      status = False
      ScreenOutput('Ping Test Core1', 'Unsuccessful')
      time.sleep(3)
    return status
...
def runTest() :
    ...
    try:
        procId = subprocess.run(["iperf3", "-c", hostname, "-p", hostport, "-J", "-t", "15", "-Z", "-A", "1" ], stdout=subprocess.PIPE, timeout=30)
        print hostname
    ...
    try:
        procId = subprocess.run(["iperf3", "-c", hostname, "-p", hostport, "-J", "-t", "15", "-Z", "-R", "-A", "1" ], stdout=subprocess.PIPE, timeout=30)
        print hostname
    ##Raise an error if the timeout expires and re-run the test
    except subprocess.TimeoutExpired:
        ScreenOutput('Speed Test iperf', 'Failed')
...
def executeTesting():
    ...
    ScreenOutput('Starting', 'Speed Test Core1')
    time.sleep(2)
