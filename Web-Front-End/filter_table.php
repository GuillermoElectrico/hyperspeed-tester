...
if ($date_search)
{
  $sql = ("SELECT * FROM test_logs_download INNER JOIN engineer_assignment ON test_logs_download.board_id = engineer_assignment.board_id WHERE connecting_to LIKE $value_pattern OR test_duration LIKE $value_pattern OR mbps_sent LIKE $value_pattern OR mbps_received LIKE $value_pattern OR test_logs_download.board_id LIKE $value_pattern OR engineer_name LIKE $value_pattern OR engineer_email LIKE $value_pattern OR file_hash LIKE $value_pattern OR timestamp BETWEEN $timestamp_entered AND $value_pattern_range ORDER BY timestamp DESC");
}
else
{
